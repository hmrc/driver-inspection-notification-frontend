...
echo "Running git navigation"

if [ $# -lt 2 ]
  then
    echo "Not enough arguments supplied"
    echo "Usage: get-current-message-files-git-navigation.sh [git commit ref] [current git branch]"
    echo "example: get-current-message-files-git-navigation.sh main RORO-00 "
    exit 1
fi
...
translationDir=${PWD}/test-resources
cd ..
hmrcPath=${PWD}
...
echo Cloning may take a moment ...
...
sbt -Dcontent.useCheckout=false clean content:test