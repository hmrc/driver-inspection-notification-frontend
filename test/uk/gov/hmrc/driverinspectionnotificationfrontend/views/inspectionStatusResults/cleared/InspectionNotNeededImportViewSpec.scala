...
class InspectionNotNeededImportViewSpec extends ViewBehaviours {

  "/search results inspection not needed import page" should {

    val view = inspectionNotNeededImportView(None)
    val doc  = view.asDocument

    behave like normalPage("no_inspection_needed_page_import.title", section = None)(view)
    ...
    "be rendered" should {
      ...
      "and show content as expected" in {
        ...
        inspectionHeading2.text()                      should be("If you are ending a transit movement")
        inspectionHeading2.nextElementSibling().text() should be("You must also present the goods at your nearest office of destination.")

        val inspectionHeading3 = doc.getElementsByTag("h3").get(2)
        inspectionHeading3.text()                      should be("If you have no other controls to report for")
        inspectionHeading3.nextElementSibling().text() should be("You can continue from your border location of arrival, to your destination.")
