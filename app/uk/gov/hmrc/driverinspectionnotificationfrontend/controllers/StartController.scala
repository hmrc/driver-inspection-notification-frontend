...
@Singleton
class StartController @Inject() (mcc: MessagesControllerComponents, startView: start)(implicit appConfig: AppConfig) extends FrontendController(mcc) {

  def show(): Action[AnyContent] = Action { implicit request =>
    val langList         = appConfig.supportedLanguage
    val otherLangSupport = appConfig.otherLanguageSupportEnabled
    Ok(startView(otherLangSupport, langList))
  }
